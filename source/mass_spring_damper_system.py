...
class body(object):
    ...
    def a(self,x, v, dt,F):
        ...
        m = self.mass
        return (F-k*x-c*v)/m
    def cal_pos_n_vel(self,t=10.0,dt=0.1,x0 = 0.0,v0 = 0.0,F=0.0):
    ...
    def plots(self,t,dt,x0,v0,F,save=False):
        ...
        plt.figure()
        plt.plot(time,displacement,'-bo', lw=1,)
        plt.plot(time,velocity,'-gx', lw=1,)
        plt.xlabel("time(sec)",family='sans-serif', style='italic',size=10)
        plt.legend(labels=["Displacement(m)","Velocity(m/s)"],fontsize=10)   
        plt.title("Mass Spring Damper System", family='sans-serif', style='italic',size=10)     
        if(save):
